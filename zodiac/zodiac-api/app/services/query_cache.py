...
def ensure_embeddings_table(db: Session) -> None:
    ...
    try:
        ...
        logger.error(f"Failed to ensure embeddings table: {e}")
        db.rollback()
...
def find_similar_cached_query(
    ...
    """
    Find a similar cached query using embedding similarity search.
    ...
    try:
        ...
        # Fetch all non-expired cached queries
        now = _now_utc()
        rows = db.execute(
            text(
                """
        ...
        if best_match:
            ...
            db.commit()
            logger.info(f"Cache hit: similarity={best_score:.3f}, query='{query_text[:50]}...'")
        else:
            logger.debug(f"Cache miss: no similar query found for '{query_text[:50]}...'")
