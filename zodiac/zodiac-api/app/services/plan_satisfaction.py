...
def result_matches_analytical_intent(
    ...
    # Dimensions required by ranking / which-X questions must appear even if group_by was incomplete.
    check_dims = set(group_by) | {
        d for d in dims
        if d in {"country", "customer", "material", "vendor", "industry", "currency", "supplier"}
        and (ranking or period or agg in {"SUM", "COUNT", "AVG", "MAX", "MIN"})
    }
