...
def _introspect_columns(db: Session, table_names: List[str]) -> Dict[str, Dict[str, str]]:
    """
    Load column descriptions directly from db_table_mapping.json.
    This is the primary metadata source; no DB introspection.
    """
    root = Path(__file__).resolve().parent.parent
    mapping_path = root / "db_table_mapping.json"

    if not mapping_path.exists():
        raise RuntimeError("db_table_mapping.json not found")
    ...
    for tbl in table_names:
        # Case-insensitive lookup for table in mapping
        entry = mapping.get(tbl) or next(
            (mapping[k] for k in mapping if isinstance(k, str) and k.upper() == tbl.upper()),
            None,
        )
        if not entry:
            continue
...
def _get_table_descriptions() -> Dict[str, str]:
    """
    Load table descriptions directly from db_table_mapping.json.
    This is the primary metadata source for table selection.
    """
    root = Path(__file__).resolve().parent.parent
    mapping_path = root / "db_table_mapping.json"
    ...
    for table, info in mapping.items():
        ...
        desc = meta.get("description", f"{table} SAP table") if isinstance(meta, dict) else f"{table} SAP table"
...
def _pick_tables(
    ...
    """
    Pick tables needed to answer the question. Uses db_table_mapping.json as the
    single source of truth. User knowledge is injected when provided.
    """
    table_descriptions = _get_table_descriptions()
    if not table_descriptions:
        raise RuntimeError("db_table_mapping.json has no tables or could not be loaded")
...
{knowledge_block}
You are selecting SAP-style tables from db_table_mapping.json.
Here are the available tables (use exact names as shown):
{json.dumps(table_descriptions, indent=2)}

IMPORTANT:
- You MUST only select tables that exist in the provided mapping.
- Do NOT invent SAP tables.
- Only use tables listed in the JSON metadata above.
- If a table contains the relevant business data (sales, invoices, products, vendors, costs),
  select that table.
- Every column you use must exist in the mapping.

Task:
- Choose ONLY the tables that are truly needed to answer the question.
- Use EXACT table names as they appear above (e.g. VBRP, VBRK, EKKO, EKPO, RBKP, RSEG).
- For sales/revenue: prefer VBRP + VBRK + KNA1 + MAKT.
...
{json.dumps({tbl: _get_table_descriptions().get(tbl, "") for tbl in selected_tables}, indent=2)}
...
def _json_to_sql_postgres(json_spec: Dict[str, Any], column_mappings: Dict[str, Dict[str, str]]) -> str:
    ...
    # Map logical/spec table names to actual column_mappings keys (from JSON).
    ...
    def _actual_table_name(logical: str) -> str:
        return logical_to_actual.get(logical) or logical_to_actual.get((logical or "").upper(), logical)
    ...
    for col in columns:
        ...
        # SAP exports store many numbers as TEXT; cast before aggregating.
        NUMERIC_TEXT_FIELDS = {
            "NETWR", "RMWWR", "WRBTR", "DMBTR", "MENGE",
            "DMBE2", "DMBE3",
        }
        col_upper = str(col_name).upper()
        needs_numeric_cast = col_upper in NUMERIC_TEXT_FIELDS
        ...
        while attempt <= max_retries:
            try:
                ...
                if not rows:
                    ...
                    try:
                        base_table = selected_tables[0] if selected_tables else None
                        if base_table:
                            diagnostic_sql = f'SELECT COUNT(*) FROM "{base_table}"'
                            diag_result = db.execute(text(diagnostic_sql)).fetchone()
                            total_rows = diag_result[0] if diag_result else 0
                            if total_rows and total_rows > 0:
                                logger.warning(
                                    f"🔍 Table {base_table} has {total_rows} rows but query filters removed them."
                                )
                    except Exception:
                        pass
                else:
