...
# ── Process-level TTL caches (avoids 10-15 min DB introspection on every query) ─
_AGENT_CACHE_TTL: int = 900  # 15 minutes
_table_desc_cache: Dict[str, str] = {}
_table_desc_cache_ts: float = 0.0
_introspect_col_cache: Dict[str, Any] = {}
_introspect_col_cache_ts: float = 0.0
_mapping_file_agent_cache: Optional[Dict[str, Any]] = None


def _load_mapping_file_agent() -> Dict[str, Any]:
    """Load db_table_mapping.json once per process lifetime (shared across all calls)."""
    global _mapping_file_agent_cache
    if _mapping_file_agent_cache is not None:
        return _mapping_file_agent_cache
    try:
        root = Path(__file__).resolve().parent.parent
        path = root / "db_table_mapping.json"
        if path.exists():
            with path.open("r", encoding="utf-8") as f:
                _mapping_file_agent_cache = json.load(f)
            return _mapping_file_agent_cache or {}
    except Exception as e:
        logger.warning("sap_sql_agent: could not load db_table_mapping.json: %s", e)
    _mapping_file_agent_cache = {}
    return {}
...
_SCHEMA_METADATA_FOR_LLM: Dict[str, Any] = {}
_SCHEMA_METADATA_LLM_LOADED = False


def _load_schema_metadata_for_llm() -> Dict[str, Any]:
    """Load schema_metadata_for_llm.json for domain classification, table selection, joins. Returns {} on failure."""
    global _SCHEMA_METADATA_FOR_LLM, _SCHEMA_METADATA_LLM_LOADED
    if _SCHEMA_METADATA_LLM_LOADED:
        return _SCHEMA_METADATA_FOR_LLM
    _SCHEMA_METADATA_LLM_LOADED = True
    try:
        root = Path(__file__).resolve().parent.parent
        path = root / "schema_metadata_for_llm.json"
        if path.exists():
            with path.open("r", encoding="utf-8") as f:
                data = json.load(f)
            if isinstance(data, dict):
                _SCHEMA_METADATA_FOR_LLM = data
                logger.info("schema_metadata_for_llm: loaded %d tables", len(data.get("tables", {})))
    except Exception as e:
        logger.warning("Could not load schema_metadata_for_llm.json: %s", e)
    return _SCHEMA_METADATA_FOR_LLM


def _classify_domain(question: str) -> List[str]:
    """Map user question to business categories using schema_metadata_for_llm _category_keywords."""
    meta = _load_schema_metadata_for_llm()
    keywords = meta.get("_category_keywords") or {}
    ...
    if not categories:
        categories = ["sales", "billing", "cost", "material"]
    return categories


def _get_tables_for_categories(categories: List[str], db_table_names: List[str]) -> List[str]:
    """Return table names from schema metadata that match the given categories. Prunes to 5-15 tables."""
    meta = _load_schema_metadata_for_llm()
    tables_section = meta.get("tables") or {}
    db_set = {t.upper() for t in db_table_names}
    cat_set = set(categories)
    candidates: List[str] = []
    for tbl, info in tables_section.items():
        if not isinstance(info, dict) or tbl.upper() not in db_set:
            continue
        tbl_cat = info.get("business_category") or ""
        if isinstance(tbl_cat, list):
            if cat_set & set(tbl_cat):
                candidates.append(tbl)
        elif tbl_cat in cat_set:
            candidates.append(tbl)
    seen: set = set()
    out = []
    for t in candidates:
...
def _get_enriched_table_context(tables: List[str]) -> Dict[str, Dict[str, Any]]:
    """Return per-table metadata: description, important_columns, joins, sql_hints for prompt enrichment."""
    meta = _load_schema_metadata_for_llm()
    tables_section = meta.get("tables") or {}
    out: Dict[str, Dict[str, Any]] = {}
    for t in tables:
        info = tables_section.get(t) or tables_section.get(t.upper())
        if isinstance(info, dict):
            out[t] = {
                "description": info.get("description", ""),
                "important_columns": info.get("important_columns", []),
                "joins": info.get("joins", []),
                "sql_hints": info.get("sql_hints", ""),
            }
    return out
...
def _introspect_columns(db: Session, table_names: List[str]) -> Dict[str, Dict[str, str]]:
    ...
    Results are cached per table-set for 15 minutes.
    """
    global _introspect_col_cache, _introspect_col_cache_ts
    cache_key = str(sorted(set(t.lower() for t in table_names)))
    now = time.time()
    if cache_key in _introspect_col_cache and (now - _introspect_col_cache_ts) < _AGENT_CACHE_TTL:
        logger.debug("sap_sql_agent: returning cached column introspection for %s", cache_key[:60])
        return dict(_introspect_col_cache[cache_key])

    insp = inspect(db.bind)
    mapping: Dict[str, Dict[str, str]] = {}

    # Load mapping file once per process
    mapping_file = _load_mapping_file_agent()
    ...
    if not isinstance(_introspect_col_cache, dict):
        pass  # guard
    _introspect_col_cache[cache_key] = mapping
    _introspect_col_cache_ts = time.time()
    return mapping
...
def _get_table_descriptions(db: Session) -> Dict[str, str]:
    ...
    Results are cached for 15 minutes to avoid repeated slow DB introspection.
    """
    global _table_desc_cache, _table_desc_cache_ts
    now = time.time()
    if _table_desc_cache and (now - _table_desc_cache_ts) < _AGENT_CACHE_TTL:
        logger.debug("sap_sql_agent: returning cached table descriptions (%d tables)", len(_table_desc_cache))
        return dict(_table_desc_cache)

    logger.info("sap_sql_agent: refreshing table descriptions from DB (cache expired or empty)")
    insp = inspect(db.bind)
    ...
    _table_desc_cache_ts = time.time()
    logger.info("sap_sql_agent: cached %d table descriptions", len(out))
    return out
...
def _pick_tables_adaptive(
    ...
    """
    Invoice-bot-style table selection. Uses schema_metadata_for_llm.json when available:
    1) Classify domain from question
    2) Prune tables by business category (5-15 tables)
    3) Enrich prompt with joins and important_columns
    ...
    db_table_names = list(table_descriptions.keys())
    meta = _load_schema_metadata_for_llm()
    tables_section = meta.get("tables") or {}
    use_metadata = bool(tables_section)
    candidate_tables: List[str] = []

    if use_metadata:
        categories = _classify_domain(question)
        candidate_tables = _get_tables_for_categories(categories, db_table_names)
        if candidate_tables:
            enriched = _get_enriched_table_context(candidate_tables)
            tables_block_parts = []
            for t in candidate_tables:
                desc = table_descriptions.get(t) or table_descriptions.get(t.upper()) or f"Table {t}"
                extra = enriched.get(t) or {}
                cols = extra.get("important_columns") or []
                joins = extra.get("joins") or []
                hints = extra.get("sql_hints") or ""
                parts = [f"{t}: {desc}"]
                if cols:
                    parts.append(f"  Key columns: {', '.join(cols[:8])}")
                if joins:
                    parts.append(f"  Joins: {'; '.join(joins[:5])}")
                if hints:
                    parts.append(f"  Hints: {hints[:120]}")
                tables_block_parts.append("\n".join(parts))
            tables_block = "\n\n".join(tables_block_parts)
            logger.info("_pick_tables_adaptive: domain=%s, pruned to %d tables", categories, len(candidate_tables))
        else:
            tables_block = json.dumps(table_descriptions, indent=2)
    else:
        tables_block = json.dumps(table_descriptions, indent=2)
...
- **CRITICAL — Currency in rankings (highest/top/lowest/best sales by customer/product/vendor)**: NETWR is stored in DOCUMENT currency (VBRK.WAERK). NEVER sum NETWR across mixed currencies for a ranking — JPY/KRW totals dwarf EUR/USD and the ranking comes out wrong. ALWAYS include VBRK.WAERK: either add waerk to GROUP BY and SELECT (per-currency ranking) or filter to one currency (WHERE waerk = 'EUR'). Always SELECT the currency column so the user sees which currency the amounts are in.
...
"""
    ...
    try:
        ...
        valid = {t.upper() for t in table_descriptions}
        if use_metadata and candidate_tables:
            valid = valid & {t.upper() for t in candidate_tables}
        if valid:
            return [n for n in names if n and n.upper() in valid]
...
def _generate_sql_json_adaptive(
    ...
    schema_metadata_for_tables: Optional[Dict[str, Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    """
    ...
    When schema_metadata_for_tables is provided (from schema_metadata_for_llm.json),
    includes joins and important_columns in the prompt for better SQL generation.
    """
    ...
    joins_block = ""
    if schema_metadata_for_tables:
        join_lines = []
        for t in selected_tables:
            meta = schema_metadata_for_tables.get(t) or schema_metadata_for_tables.get(t.upper())
            if isinstance(meta, dict):
                joins = meta.get("joins") or []
                cols = meta.get("important_columns") or []
                if joins or cols:
                    parts = [f"{t}: {', '.join(cols[:6])}"]
                    if joins:
                        parts.append(f"Joins: {'; '.join(joins[:5])}")
...
{semantic_block}{joins_block}User query: "{question}"
...
**Highest sales by customer:** Use only VBRK, VBRP, KNA1; select KUNNR, NAME1, SUM(NETWR), and VBRK.WAERK as currency; group by KUNNR, NAME1, WAERK (or filter to one currency) — NEVER rank summed NETWR across mixed currencies. If the question names a year, filter SUBSTRING(TRIM(VBRK.fkdat),1,4) = '<year>'. Do NOT add VBAK, VBFA, LIKP, LIPS. Order by SUM(NETWR) DESC.
...
**Top N (top 20 customers, top 10 materials, top 20 vendors):** Select the dimension (customer, material, vendor), SUM of amount/revenue; group by that dimension; order by the sum DESC; limit N (e.g. 20 or 10). Use VBRK/VBRP/KNA1 for customers, VBRP/MAKT for materials, EKPO/EKKO/LFA1 for vendors. For billing-based rankings ALWAYS include the currency column (VBRK.WAERK) in SELECT and GROUP BY, or filter to a single currency — mixed-currency sums rank wrong.
...
**Negative sales / credit memos:** Individual billing LINE ITEMS where CAST(netwr AS NUMERIC) < 0. NEVER use HAVING SUM(netwr) < 0. **Lowest sales:** ranked billed totals (VBRK/vbrp) ORDER BY total ASC LIMIT N — do not add netwr < 0.
...
def run_adaptive_sap_sql_agent(
    ...
    try:
        ...
        table_descriptions = _get_table_descriptions(db)
        schema_metadata = _get_enriched_table_context(selected_tables) if _load_schema_metadata_for_llm().get("tables") else None
        spec = _generate_sql_json_adaptive(
            question,
            selected_tables,
            column_mappings,
            ...
            few_shot_examples=few_shot_examples,
            table_descriptions=table_descriptions,
            schema_metadata_for_tables=schema_metadata,
        )
        if not spec:
...
def run_sap_sql_agent(
    ...
    # Catalog is now used as fallback from orchestrator when LLM paths fail.
    # LLM-first: schema-driven, adaptive, and standard agent run before catalog.
