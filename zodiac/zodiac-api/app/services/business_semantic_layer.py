...
METRICS: Dict[str, MetricDef] = {
    ...
    "avg_selling_price": MetricDef(
        name="avg_selling_price",
        description="Revenue / billed quantity at billing-item grain",
        aliases=("average selling price", "asp", "unit price", "selling price"),
        status="supported",
        formula_sql=(
            "SUM(CAST(NULLIF(TRIM(v.netwr), '') AS NUMERIC)) "
            "/ NULLIF(SUM(CAST(NULLIF(TRIM(v.fkimg), '') AS NUMERIC)), 0)"
        ),
        base_tables=("vbrp",),
        grain="billing_item",
        currency_field="vk.waerk",
        time_field="vk.fkdat",
        default_dimensions=("product",),
        caveats="ASP from billing NETWR/FKIMG. Not list price or KONV condition price.",
    ),
    "purchase_value": MetricDef(
        name="purchase_value",
        description="Purchase order item net value (EKPO.NETWR) — not invoice COGS",
        aliases=("purchase value", "purchase cost", "po value", "procurement value"),
        status="partial",
        formula_sql="SUM(CAST(NULLIF(TRIM(ekpo.netwr), '') AS NUMERIC))",
        base_tables=("EKPO", "EKKO"),
        grain="po_item",
        currency_field="ekko.waers",
        time_field="ekko.bedat",
        default_dimensions=("product", "supplier"),
        caveats="PO value is not WAVWR COGS. Material-only link to billed products.",
        substitutes_forbidden=("cogs", "logistics cost"),
    ),
    "inventory_value": MetricDef(
        name="inventory_value",
        description="Material valuation stock value (MBEW.SALK3)",
        aliases=("inventory value", "stock value", "inventory"),
        status="partial",
        formula_sql="SUM(CAST(NULLIF(TRIM(mbew.salk3), '') AS NUMERIC))",
        base_tables=("MBEW",),
    ...
    "delivery_count": MetricDef(
        name="delivery_count",
        description="Count of delivery documents — activity, not logistics cost",
        aliases=("delivery count", "shipments", "delivery quantity"),
        status="partial",
        formula_sql="COUNT(DISTINCT likp.vbeln)",
        base_tables=("LIKP",),
        grain="delivery_header",
        currency_field=None,
        time_field="likp.lfdat",
        default_dimensions=("product",),
        caveats="Delivery activity only. Never treat as freight cost.",
        substitutes_forbidden=("logistics cost",),
    ),
    "discount": MetricDef(
        name="discount",
        description="Pricing discount from KONV",
        aliases=("discount", "discounts"),
        status="unavailable",
        formula_sql="",
        base_tables=("KONV",),
        grain="condition_line",
        currency_field=None,
        time_field=None,
        default_dimensions=(),
        caveats="VBRK.KNUMV→KONV exists but KSCHL discount vs tax vs freight is not certified.",
    ),
