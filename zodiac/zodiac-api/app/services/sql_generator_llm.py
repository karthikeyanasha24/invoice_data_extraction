...
- For sales/revenue: use VBRK (header), VBRP (items); join on VBELN with LPAD; NETWR must be CAST(TRIM(NETWR) AS NUMERIC); FKDAT is billing date (TEXT, format YYYYMMDD). CRITICAL customer field: ALWAYS use VBRK.KUNAG (sold-to party = the actual buying customer) — NEVER use VBRK.KUNRG (payer) for customer sales ranking. KUNRG is a paying intermediary (bank/parent company) and will collapse dozens of different customers into one entity, producing completely wrong rankings. Customer join: LPAD(TRIM(VBRK.kunag), 10, '0') = LPAD(TRIM(KNA1.kunnr), 10, '0'). VBELN join: LPAD(TRIM(VBRK.vbeln), 10, '0') = LPAD(TRIM(VBRP.vbeln), 10, '0'). Both LPAD calls required on both sides — without them, leading-zero mismatches silently drop rows.
