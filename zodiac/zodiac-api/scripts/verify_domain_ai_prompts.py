"""
Cross-check DOMAIN_QUERY_EXAMPLES (What can you ask?) against operational_query_resolver.

Usage (from zodiac-api):
  python scripts/verify_domain_ai_prompts.py

Keep DOMAIN_ROWS in sync with:
  zodiac-front/src/components/DashboardAIAnalysis.tsx  →  DOMAIN_QUERY_EXAMPLES
  tests/test_domain_ai_prompts.py                      →  DOMAIN_ROWS
"""
...
# Mirror tests/test_domain_ai_prompts.py DOMAIN_ROWS
...
def main() -> int:
    failed = False
    for category, question, expected in DOMAIN_ROWS:
        ...
        if not ok:
            failed = True
        print(f"[{status}] {category}: operational={got!r} (expected {expected!r})")
