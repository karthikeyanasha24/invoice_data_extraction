...
def reset_invoices(include_business_data: bool = True):
    """
    ...
    try:
        logger.info("=" * 60)
        logger.info("🧹 RESET INVOICE TABLES")
        logger.info("=" * 60)
        ...
        logger.info(f"   - {counts_before.get('Successful Invoices', 0)} successful invoice(s)")
        logger.info(f"   - {counts_before.get('Failed Invoices', 0)} failed invoice(s)")
        if include_business_data:
        ...
        logger.info(f"   Deleted {failed_deleted} failed invoice(s)")
        if include_business_data:
            logger.info(f"   Deleted business intelligence records")
        logger.info("=" * 60)
        
        logger.info("\n🚀 Ready for fresh invoice testing!")
        logger.info("\n📋 NEXT STEPS:")
        logger.info("   1. Restart API server: uvicorn app.server:app --reload")
        ...
        logger.info("      • Duplicate invoice numbers are blocked")
        logger.info("=" * 60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Reset invoice tables for fresh testing')
    parser.add_argument(
        '--keep-business-data',
        action='store_true',
    ...
    reset_invoices(include_business_data=not args.keep_business_data)
