...
export default function ProcessingStatusTracker({
    ...
    // Don't use hardcoded expected steps - only show steps actually returned from backend
    // This way X12 files (3 steps) don't show XML pipeline steps (6 steps)
        ...
        <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-3">
                {displaySteps.map((step) => {
                    ...
                    // Check if step is skipped (success but message contains "skipped")
                    const isSkipped = step?.success === true &&
                        (step?.message?.toLowerCase().includes('skipped') ||
                            step?.message?.toLowerCase().includes('skip'));
